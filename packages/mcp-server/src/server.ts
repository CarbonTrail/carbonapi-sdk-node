...
import { ClientOptions } from 'carbonapi-node';
...
export { ClientOptions } from 'carbonapi-node';
export { endpoints } from './tools';

// Create server instance
export const server = new McpServer(
  {
    name: 'carbonapi_node_api',
...
export function initMcpServer(params: {
  ...
  const client = new CarbonAPI(params.clientOptions);
  const capabilities = {
    ...defaultClientCapabilities,
...
export function init(params: {
  ...
  const client = params.client || new CarbonAPI({ defaultHeaders: { 'X-Stainless-MCP': 'true' } });
